...
def signup(db: Session, signup: user_schema.UserSignupRequest)->user_model.User | ValueError:
    ...
    user:user_model.User = user_model.User(username=signup.username, email=signup.email, hashed_password=hashed_password, display_name=signup.display_name, is_admin=signup.is_admin)
...
def login(db: Session, username: str, password: str)->user_schema.Token | ValueError:
    ...
    if user_crud.exist_refresh_token_by_username(db=db, username=user.username):
        user_crud.update_refresh_token(db=db, username=user.username, refresh_token=refresh_token)
    else:
        refresh_token_obj = user_model.RefreshToken(user_username=user.username, refresh_token=refresh_token)
        user_crud.create_refresh_token(db=db, refresh_token=refresh_token_obj)
    return user_schema.Token(access_token=access_token, refresh_token=refresh_token, token_type="bearer")

def refresh_access_token(db: Session, refresh_token: str)->user_schema.Token | ValueError:
    try:
    ...
    if username is None or exp is None:
        raise ValueError("Decoding failed")
    if not user_crud.is_refresh_token_valid(db=db, refresh_token=refresh_token, username=username):
