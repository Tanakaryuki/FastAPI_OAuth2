...
def exist_refresh_token_by_username(db: Session, username: str) -> bool:
    db_token = db.query(user_model.RefreshToken).filter(user_model.RefreshToken.user_username == username).first()
    return db_token is not None

def create_refresh_token(db: Session, refresh_token: user_model.RefreshToken) -> user_model.RefreshToken | None:
